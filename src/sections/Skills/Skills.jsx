              ...
              {group.items.map((item) => (
                <div key={item.name} className="skills-item">
                  <div className="skills-meta">
                    <span>{item.name}</span>
                    <span aria-label={`Proficiency: ${item.level} percent`}>{item.level}%</span>
                  </div>
                  <div className="skills-bar" role="progressbar" aria-valuenow={item.level} aria-valuemin="0" aria-valuemax="100" aria-label={`${item.name} proficiency`}>
